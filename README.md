...
| 分区消费记录视图、首页储蓄分区独立展示 | ✅ |
| **自动识别记账：App 内相册选付款截图本机 OCR 识别 + 银行短信快捷指令识别 → 预填确认单** | ✅ |
...
1. 支付完成后，截一张「付款成功」页面；
2. 打开 App 首页 → 点右下角悬浮的**「截图识别」**按钮 → 从相册选择这张截图；
3. App 在**本机 OCR 识别**金额、商家、时间（Vision 框架，离线）；
4. 自动弹出确认单（金额/内容/日期/推荐分区都已填好）→ 点「确认入账」→ 预算自动扣减。

识别不出金额会明确提示；全程不经过任何服务器。
...
- 截图识别完全在 App 内完成（PhotosPicker 选图 → Vision OCR → 确认单），无任何跨进程通道；解析器（OCR 文本行 → 金额/商家/时间；短信 → 金额/银行/渠道）是纯函数，有完整单元测试，持续按真实截图样本迭代；
